    ...
    case MODEM_LORA:
        {
            ...
            SX1276.Settings.LoRa.RxContinuous = rxContinuous;  /* 持续/单一接收模式 */
            ...
            if( SX1276.Settings.LoRa.FreqHopOn == true )
            {
                ...
                SX1276Write( REG_LR_HOPPERIOD, SX1276.Settings.LoRa.HopPeriod );
						...
						/* SX1278 频率不超过 RF_MID_BAND_THRESH */
            ...
            {
                // ERRATA 2.1 - Sensitivity Optimization with a 500 kHz Bandwidth
								/* sx1278-LoRa模式这个寄存器(0x36)保留  */
                SX1276Write( REG_LR_TEST36, 0x03 );
            }

						/* sx1278-LoRa模式这两个寄存器(0x31), (0x37)保留  */
            if( datarate == 6 )
            {
                        ...
                        uint8_t hopPeriod, bool iqInverted, uint32_t timeout )
{
    ...
    //paConfig = ( paConfig & RF_PACONFIG_MAX_POWER_MASK ) | 0x70;
		...
		/* 默认使用 PABOOST 引脚，MaxPower = 0,  OutputPower = 0 */
		paConfig =0x80;	
		
		/* 检查是否使用的默认使用 PABOOST 引脚 */ 
    if( ( paConfig & RF_PACONFIG_PASELECT_PABOOST ) == RF_PACONFIG_PASELECT_PABOOST )
    {
        if( power > 17 )
        {
						/* 当 OutputPower = 1111 时，在 PA_BOOST 上为 +20dBm */
    ...
    case MODEM_LORA:
        {
            ...
            bandwidth += 7; 
            SX1276.Settings.LoRa.Bandwidth = bandwidth;     /* 带宽 */
            SX1276.Settings.LoRa.Datarate = datarate;       /* 扩频因子 */
            SX1276.Settings.LoRa.Coderate = coderate;       /* 编码率 */
            SX1276.Settings.LoRa.PreambleLen = preambleLen; /* 前导码长度 */
            SX1276.Settings.LoRa.FixLen = fixLen;           /* 显式/隐式 报头 */
            SX1276.Settings.LoRa.FreqHopOn = freqHopOn;     /* 跳频开启/关闭 */
            SX1276.Settings.LoRa.HopPeriod = hopPeriod;     /* 跳频周期 */
            SX1276.Settings.LoRa.CrcOn = crcOn;             /* CRC开启/关闭 */
            SX1276.Settings.LoRa.IqInverted = iqInverted;   /* ? */
            SX1276.Settings.LoRa.TxTimeout = timeout;       /* 发送超时时间 */
...
{
    bool rxContinuous = false;	/* 接收模式初始化，这个值没有意义 */
    ...
    case MODEM_LORA:
        {
						/* IqInverted = false, 未使用此功能 */
            if( SX1276.Settings.LoRa.IqInverted == true )
            {
								/* SX1278中这两个寄存器 (0x33), (0x3B) 保留 */
            ...
            {
						    /* 屏蔽 接收到有效报头、发送完成、CAD完成、FHSS改变信道、检测到CAD 中断 */
                SX1276Write( REG_LR_IRQFLAGSMASK, //RFLR_IRQFLAGS_RXTIMEOUT |
                                                  //RFLR_IRQFLAGS_RXDONE |
                                                  //RFLR_IRQFLAGS_PAYLOADCRCERROR |
                                                  RFLR_IRQFLAGS_VALIDHEADER |
                ...
                SX1276Write( REG_DIOMAPPING1, ( SX1276Read( REG_DIOMAPPING1 ) & RFLR_DIOMAPPING1_DIO0_MASK ) | RFLR_DIOMAPPING1_DIO0_00 );
						...
						/* FIFO 数据缓冲区中接收解调器的读取基地址 */
            SX1276Write( REG_LR_FIFORXBASEADDR, 0 );
						/* FIFO 数据缓冲区中 SPI 接口地址指针 */
            SX1276Write( REG_LR_FIFOADDRPTR, 0 );
        }
        break;
    }
